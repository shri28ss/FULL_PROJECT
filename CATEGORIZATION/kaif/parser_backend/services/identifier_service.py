...
# ════════════════════════════════════════════════════════════
# FORMAT DEDUPLICATION HELPERS (OLD - KEPT FOR COMPATIBILITY)
# ════════════════════════════════════════════════════════════
...
def find_existing_identifier(text: str, default_threshold: float = 80.0):
    ...
    logger.info("Format matching: checking %d stored formats (threshold=%.1f)", len(categories), default_threshold)
    ...
    logger.info("Document text (first 200 chars, cleaned): %s", doc_clean[:200])
    ...
    for cat in categories:
        ...
        raw_inst   = cat.get("institution_name") or ""
        inst_norm  = normalise_institution_name(raw_inst)          # → "HDFC BANK"
        inst_clean = re.sub(r"\s+", "", inst_norm.lower())         # → "hdfcbank"
        doc_family = cat.get("document_family", "")
        is_generic = (
            (not inst_clean or inst_clean == "unknown") and doc_family in _GENERIC_FAMILIES
        )
        ...
        logger.info(
            "Checking format: statement_id=%s institution='%s' normalized='%s' family=%s",
            cat.get("statement_id"), raw_inst, inst_clean, doc_family
        )

        if is_generic:
            logger.info("  → Added to generic candidates")
            generic_candidates.append(cat)
            continue
        if inst_clean and inst_clean != "unknown" and inst_clean not in doc_clean:
            logger.info("  → FILTERED OUT: institution name '%s' not found in document", inst_clean)
            continue
        logger.info("  → Added to specific candidates")
        specific_candidates.append(cat)

    logger.info("Candidates: %d specific, %d generic", len(specific_candidates), len(generic_candidates))
    ...
    def score_pool(pool: List[Dict], score_penalty: float = 0.0) -> List[Dict]:
        ...
        for cat in pool:
            ...
            logger.info(
                "Format match check: statement_id=%s institution=%s status=%s score=%.2f threshold=%.2f match=%s",
                cat.get("statement_id"),
                cat.get("institution_name"),
                cat.get("status"),
                adjusted_score,
                target_threshold,
                "YES" if adjusted_score >= target_threshold else "NO"
            )
    ...
    matches = score_pool(specific_candidates, score_penalty=0.0)
    if not matches and generic_candidates:
        logger.info("No specific matches found, trying generic candidates with 15pt penalty...")
        matches = score_pool(generic_candidates, score_penalty=15.0)

    if not matches:
        logger.info("No format matched above threshold - will create new format")
        return False, None
